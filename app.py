...
DATABASE = '/home/ubuntu/server/example.db'
FILENAME = '/home/ubuntu/server/file_download.txt'
...
def registration():
    message = ''
    result = execute_query("""SELECT name FROM sqlite_master WHERE type='table' AND name=(?);""", ("users", ))
    if result:
        print("Table exists")
    else:
        result = execute_query("""CREATE TABLE users (Username text,Password text,firstname text, lastname text, email text, count integer)""")
...
def responsePage(firstname, lastname, email, count):
    return ("""<div align="center"><h1>Successful Transaction!!</h1></div><body><br> First Name :  """ + str(firstname) + """ <br> Last Name : """ + str(lastname) + """ <br> Email : """ + str(email) + """ <br> Word Count : """ + str(count) + """ <br><br> <a href="/download" >Download</a> </body>""")
